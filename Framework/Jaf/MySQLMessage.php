 ...
 * MySQL异常信息本地化翻译
 */
class Jaf_MySQLMessage {

	public static $messages;
	public static $states = [];
	/**
	 * 由 errorInfo 返回
	 * @param  array $errorInfo 原始errorInfo数组
	 * @param  string $lang default zh-CN 语言
	 * @return string 翻译后的信息
	 */
	public static function errorInfo(array $errorInfo,$lang='zh-CN'){
		list($state,$code,$msg) = $errorInfo;
		if(!$msg || !isset(self::$messages[$code])) return $msg;
		$state = self::$messages[$code][1];
		self::lang($lang);
		$states=self::$states[$lang];
		if(!isset( $states[$state]) ) return $code.':'.$msg;
		$tmp = self::$messages[$code][2];
		$cnt=substr_count($tmp,'%');
		$a = sscanf($msg,$tmp);
		foreach ($a as $value) {
			if($value) $cnt--;
		}
		if($cnt!=0) return $code.':'.$msg;
		return str_replace(['%0','%1','%2','%3','%4'],$a,$states[$state]);
	}
	/**
	 * 调入指定语言翻译
	 * @param  string $lang='zh-CN' [description]
	 * @return [type]               [description]
	 */
	public static function lang($lang='zh-CN'){
		if(isset(self::$states[$lang])) return;
		$file= __DIR__. "/MySQLMessage/$lang.php";
		if( !file_exists( $file )) {
			return;
		}
		require $file;
	...
	/**
	 * 输出原始message模板和翻译对照,以方便修改
	 * @param  string $lang='zh-CN'
	 * @return string JSON 字符串
	 */
	public static function diff($lang='zh-CN'){
		self::lang($lang);
