...
trait Trait_Filter {
 ...
 private $_trait_filter=array();
 /**
  * 根据 name 匹配到 $_trait_filter 对数据进行校验过滤
  * @note 数据被引用传递,如果 $name 过滤器返回 false 结束后续过滤
  * @return boolean
  */
 public function nameValueFilter ($name , &$value){
	if(!isset($this->_trait_filter[$name])) return true;
	return array_walk( $this->_trait_filter[$name] , function ( $methods ) use($value){
 ...
 public function setFilter($name,$filterMethod){
 	$this->_trait_filter[$name] = array_merge($this->_trait_filter[$name]?:[],$filterMethod);
 }
}